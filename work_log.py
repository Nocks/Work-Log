...
def show_menu():
    ...
    while True:
        ...
        if menu_option.upper() in "ALQ":
            ...
            input("\n Invalid option. Please use the menu. Enter either 'A', 'L', or 'Q'. Press enter to continue... ")
            start_work_log()
            break
        ...
        if menu_option.upper() in "DTRH":
            if menu_option.upper() == "D":
                ...
                if not date_search_method:
                    input("\n Kindly enter how you want to search. Press enter to continue... ")
                    continue
                if date_search_method.upper() not in "RS":
                    input("\n Kindly enter either 'S' or 'R'. Press enter to continue... ")
                    continue
                else:
...
def search_by_specific_date():
    ...
    while True:
        ...
        if not search_date:
            input("\n Kindly enter a date. Press enter to continue... ")
            continue
        ...
        if search_date not in entry_dates:
            input("\n Kindly enter valid date. Pick a date from the above. Press enter to continue... ")
            continue
        else:
...
def search_by_date_range():
    ...
    while True:
        ...
        if not re.match(r'[0-9]{2}-[0-9]{2}-[0-9]{4}\s?to\s?[0-9]{2}-[0-9]{2}-[0-9]{4}$', date_range):
            input("\n Invalid date range. Date must be in the right format. Press enter to continue... ")
            continue
        else:
